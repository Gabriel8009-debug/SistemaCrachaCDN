...
class Sidebar(ctk.CTkFrame):
    def __init__(self, master, ao_navegar):
        ...
        ctk.CTkFrame(self, height=1, fg_color="#21486F").grid(
            row=1, column=0, padx=16, pady=(0, 12), sticky="ew"
        )

        itens = [
            ("dashboard", "home", "Início"),
            ("producao", "badge", "Produção"),
            ("reimpressao", "refresh", "Reimpressão"),
            ("historico", "history", "Histórico"),
            ("configuracoes", "gear", "Configurações"),
        ]

        for indice, (chave, icone_nome, texto) in enumerate(itens, start=2):
            ...
            imagem = Image.open(base / f"{icone_nome}.png")
            icone = ctk.CTkImage(
                light_image=imagem,
                dark_image=imagem,
                size=(18, 18),
            )
            self.icones[chave] = icone
